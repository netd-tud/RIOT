                                         ...
                                         size_t *mac_length)
{
    ...
    required_mac_length = PSA_MAC_LENGTH(attributes->type, attributes->bits, PSA_ALG_SHA_512);
    if (mac_size < required_mac_length) {
        return PSA_ERROR_BUFFER_TOO_SMALL;
    }

    ret = CRYS_HMAC(CRYS_HASH_SHA512_mode, (uint8_t *)key_buffer, key_buffer_size, (uint8_t *)input,
